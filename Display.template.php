...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		{
			echo '<strong class="att">'.$txt['Text_21'].':</strong>
		...
		if ($settings['show_modify'] && !empty($message['modified']['name']))
			echo '
								&#171; <em>', $txt['last_edit'], ': ', $message['modified']['time'], ' ', $txt['by'], ' ', $message['modified']['name'];
		
		// Dirty yet beautiful edits.
		if ($settings['show_modify'] && !empty($message['modified']['name']) && $message['can_rlem'])
			echo '; <a href="', $scripturl, '?action=unsetedittime;post=', $message['id'], ';topic=', $context['current_topic'], ';uid=', $message['id_member'], '">', $txt['remove_edit_sign'], '</a>';
